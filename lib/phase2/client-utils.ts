...
// Helper functions for interacting with Phase 2 APIs from client components
...
/**
 * Fetch images with Phase 2 status information
 */
 ...
 * (Assuming assets are served from /public/storage/)
 */
export function getAssetUrl(relativePath: string): string {
  if (!relativePath) return '';
  
  // Convert absolute path to URL path
  // /tmp/storage/thumbnails/[imageId]/thumb-128.jpg → /storage/thumbnails/[imageId]/thumb-128.jpg
  ...
  return new Promise((resolve, reject) => {
    const poll = async () => {
      try {
        ...
        // Stop polling when image is stored or failed
